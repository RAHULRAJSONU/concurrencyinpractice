...
public class ImageRecolourProcessor {
  ...
  public void recolorMultiThreated(BufferedImage originalImage, BufferedImage resultImage,
      int numberOfThreads) {
  ...
  public void apply(boolean multiThreaded) throws IOException {
    BufferedImage originalImage = ImageIO.read(new File(SOURCE_FILE));
    BufferedImage resultImage = new BufferedImage(originalImage.getWidth(),originalImage.getHeight(),BufferedImage.TYPE_INT_RGB);
    if (multiThreaded) {
      recolorMultiThreated(originalImage, resultImage, 3);
    } else {
